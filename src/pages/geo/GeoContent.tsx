...
function downloadMd(filename: string, content: string) {
  ...
  a.href = url;
  a.download = `${filename.replace(/[^\w\sа-яё-]/gi, '').slice(0, 60)}.md`;
  a.click();
          ...
          {drafts.map((d) => (
            <DraftCard
              ...
              onExport={async () => {
                ...
                downloadMd(d.title, r.draft.content_md);
              }}
            />
          ))}
        </div>
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <Button variant="outline" size="sm" onClick={() => downloadMd(title, content)}>
